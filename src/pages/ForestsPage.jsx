...
export default function ForestsPage() {
  ...
  const handleAddForest = (e) => {
    ...
    if (!newForestName.trim() || !newForestAbbr.trim()) {
      setIsShaking(true);
      setTimeout(() => setIsShaking(false), 500);
      showError('Please fill in all required forest details.');
      return;
    }
    ...
    const createdForest = {
      ...
      zone: newForestZone,
      location: newForestLocation.trim() || 'Forest Reserve',
      sensors: {
        tree: [
    ...
    showSuccess(`Forest Reserve "${createdForest.name}" registered successfully!`);
  };
      ...
      {activeTab === 'add' ? (
        ...
        <div className={`bg-white p-6 rounded-xl border border-slate-200 shadow-sm max-w-2xl mx-auto space-y-4 ${isShaking ? 'animate-shake' : ''}`}>
          ...
          <form onSubmit={handleAddForest} className="space-y-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 text-xs bg-slate-50 border rounded focus:outline-none transition-colors ${
                  formTouched && !newForestName.trim()
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 text-xs bg-slate-50 border rounded focus:outline-none uppercase font-mono transition-colors ${
                    formTouched && !newForestAbbr.trim()
                      ? 'border-red-500 bg-red-50/30'
                      : newForestAbbr.trim()
                      ? 'border-emerald-500 bg-emerald-50/20'
                      : 'border-slate-300'
            ...
            >
              ...
              Add Forest Reserve to System
            </button>
          </form>
        </div>
      ) : activeTab === 'zones' ? (
        /* ZONE MANAGEMENT SUBTAB */
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6 card-hover">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {['North Zone 2', 'South Zone 2', 'East Zone 2', 'West Zone 2'].map((z) => (
              <div key={z} className="p-4 bg-slate-50 border border-slate-200 rounded-lg card-hover">
                <span className="text-xs font-bold text-slate-900 block">{z}</span>
                <span className="text-2xl font-extrabold text-[#1B4332] block mt-1">12 Telemetry Nodes</span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-6 card-hover">
              ...
              <div className="flex items-center gap-3">
                <span className={`text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1.5 ${
                  selectedSensor.status === 'Online'
                    ? 'bg-emerald-100 text-emerald-800 border border-emerald-300 status-glow-online'
                    : 'bg-red-100 text-red-800 border border-red-300 status-glow-offline'
                }`}>
            ...
            <div className="p-6 bg-[#1B4332] rounded-xl shadow-inner flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-white">
              <div>
                ...
                <p className="text-xs text-emerald-200/80 mt-1">
                  Optimal operational threshold verified for forest canopy
                </p>
              </div>
                ...
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <Tooltip text={`Tree Sensor · ${selectedForest.name} · ${selectedForest.zone}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {selectedForest.sensors.soil.map((sensor) => (
                ...
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <Tooltip text={`Soil Sensor · ${selectedForest.name} · ${selectedForest.zone}`}>
                        <span className="text-xs font-mono font-bold text-slate-900 bg-slate-200 px-1.5 py-0.5 rounded hover:bg-slate-300 transition-colors cursor-help">
                          {sensor.id}
                        </span>
                      </Tooltip>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {selectedForest.sensors.water.map((sensor) => (
                ...
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <Tooltip text={`Water Sensor · ${selectedForest.name} · ${selectedForest.zone}`}>
                        <span className="text-xs font-mono font-bold text-slate-900 bg-slate-200 px-1.5 py-0.5 rounded hover:bg-slate-300 transition-colors cursor-help">
                          {sensor.id}
                        </span>
                      </Tooltip>
